...
After a workspace has been observed, OpenClaw also keeps a sibling attestation marker next to the workspace path. If a recently attested workspace disappears or is wiped, startup refuses to silently re-seed `BOOTSTRAP.md`; restore the workspace or remove the attestation marker when the reset was intentional.
